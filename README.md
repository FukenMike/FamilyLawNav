...
- All 50 states + DC are scaffolded.  GA and AL currently have expanded
  curated content in `tools/pack-builder/sources`.
- Packs are generated by the pack-builder tool into `public/packs/`; there is
  no manual generation of `data/statePacks` files or reliance on `quality`
  flags.  Any `sources` file can set `quality` if desired, but the builder
...
The pack-building tooling lives under `tools/pack-builder`.
Every execution will iterate over a **static list of 50 states plus DC** defined in
`config.js`, load the corresponding file from `tools/pack-builder/sources/<STATE>.js`,
and emit a normalized JSON pack in `public/packs` along with an updated manifest.

Each source file may supply a `jurisdiction_sources` object and any pack content
(domains, authorities, etc.).  Missing source files are tolerated; the builder
still writes an empty but valid shell pack for the state.

Builds are deterministic except for the `packVersion` timestamp, and the
validation step runs automatically during generation.

You no longer need to parse or maintain `data/statePacks`; that export exists
only for application runtime imports if any, but the builder ignores it.
