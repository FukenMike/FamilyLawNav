...
const packCacheKey = (
  ...
  schemaVersion: string,
  packVersion: string,
) => `packStore_pack_${state}_${schemaVersion}_${packVersion}`
...
function validateManifest(x: any): { ok: boolean; error?: string } {
  if (!x || typeof x !== 'object')
    return { ok: false, error: 'manifest not object' }
  if (typeof x.schemaVersion !== 'string')
    return { ok: false, error: 'schemaVersion missing or not string' }
  if (!x.packs || typeof x.packs !== 'object')
    return { ok: false, error: 'packs missing or not object' }
  for (const state of Object.keys(x.packs)) {
    const entry = x.packs[state]
    if (!entry || typeof entry.packVersion !== 'string')
      return { ok: false, error: `invalid pack entry for ${state}` }
    if (entry.sha256 && typeof entry.sha256 !== 'string')
      return { ok: false, error: `invalid sha256 for ${state}` }
  }
  return { ok: true }
}
...
function validateStatePack(p: any): { ok: boolean; error?: string } {
  ...
  if (typeof p.jurisdictions_sources.code !== 'string')
    return {
      ok: false,
      error: 'jurisdictions_sources.code missing or not string',
    }
  if (typeof p.jurisdictions_sources.rules !== 'string')
    return {
      ok: false,
      error: 'jurisdictions_sources.rules missing or not string',
    }
  if (typeof p.jurisdictions_sources.opinions !== 'string')
    return {
      ok: false,
      error: 'jurisdictions_sources.opinions missing or not string',
    }
  if (
    p.jurisdictions_sources.forms != null &&
    typeof p.jurisdictions_sources.forms !== 'string'
  ) {
  ...
  if (!Array.isArray(p.legalTests))
    return { ok: false, error: 'legalTests not array' }
  if (!Array.isArray(p.testItems))
    return { ok: false, error: 'testItems not array' }
...
export async function getManifest(opts?: {
  ...
  const cachedWrapper = (await getLocal(manifestCacheKey)) as {
    manifest: Manifest
    cachedAt: string
  } | null
  let manifest: Manifest | null = cachedWrapper?.manifest ?? null
  const now = Date.now()
  const age = cachedWrapper
    ? now - Date.parse(cachedWrapper.cachedAt)
    : Infinity
  ...
  if ((opts?.force || expired) && MANIFEST_URL) {
    try {
      ...
      manifest = candidate
      await setLocal(manifestCacheKey, {
        manifest: candidate,
...
export async function getCachedPack(
  state: string,
): Promise<{ pack: StatePack | null; status: PackStatus }> {
  const statusBase: PackStatus = {
    state,
    source: 'none',
    lastTriedAt: new Date().toISOString(),
  }
...
export async function clearCachedPack(state: string): Promise<void> {
  ...
  const manifestCached = (await getLocal(manifestCacheKey)) as {
    manifest: Manifest
  } | null
  const keys: string[] = []
  ...
  if (manifestVersion && manifestSchema)
    keys.push(packCacheKey(state, manifestSchema, manifestVersion))
  keys.push(packCacheKey(state, 'unknown', 'seed'))
  if (isWeb && typeof window !== 'undefined' && window.localStorage) {
    for (let i = 0; i < window.localStorage.length; i++) {
      const k = window.localStorage.key(i) || ''
...
export async function getPack(
  ...
  if (cached.pack) {
    // always adapt any cached pack before handing it back; this ensures
    // later code can rely on the canonical v1 schema regardless of origin.
    cached.pack = adaptPackToV1(cached.pack)

    const isFreshCache =
      !cached.status.isStale &&
      cached.status.packVersion === manifestPackVersion
    if (isFreshCache) {
